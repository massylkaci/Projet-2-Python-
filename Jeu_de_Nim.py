...
def Nim():
	...
	a = input( 'Joueur 1, Entrez votre nom : ' )
	b = input( 'Joueur 2, Entrez votre nom : ' )
	...
	print(' Il faut entrer les coups sous la forme "Numéro du tas" - "Nombre de pierres à retirer" ')	
	...
	while sum( tas.values() ) > 1 :
		...
		if min(t, p) < 0 or t > len(tas) :
			print('Faux!, veillez entrer un coups sous la forme "Numéro du tas" - "Nombre de pierres à retirer" ')
			continue
		if p > tas[t] :
			print('Faux!, veillez entrer un coups sous la forme "Numéro du tas" - "Nombre de pierres à retirer" ')
			continue
	...
	x = input ("Cette Partie est terminee, voulez-vous commencer une nouvelle partie ? [oui/non] : ")
	if x == 'oui' :
		Nim()
